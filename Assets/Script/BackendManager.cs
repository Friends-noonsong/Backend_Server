    ...
    async void Test()
    {
        await Task.Run(() =>
        {
            ...
            */
            ...
            // [추가] 닉네임 변경
            BackendLogin.Instance.UpdateNickname("유저3");
            */
            ...
            */
            ...
            /*
            // user3에게 보낼 것이므로 user1로 회원가입
            string user1Id = "user1";

            // user1Id로 회원가입
            // (409 에러 발생 시, 이미 user1Id로 아이디를 생성했으므로
            // CustomSignUp을 CustomLogin으로 변경)
            ...
            // 유저3의 닉네임(유저에 따라 다를 수 있습니다.)
            string user3Nickname = "유저3";
            // 친구 요청 보내기 함수
            BackendFriend.Instance.SendFriendRequest(user3Nickname);
            */

            /*
            // Step2에서 user1가 친구요청을 보낸 유저로 로그인해야합니다.
            BackendLogin.Instance.CustomLogin("user3", "1234");
